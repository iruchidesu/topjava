...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public List<User> getAll() {
        ...
        Map<Integer, Set<Role>> mapUserIdRoles = new HashMap<>();
        jdbcTemplate.query("SELECT * FROM user_roles", resultSet -> {
            mapUserIdRoles.computeIfAbsent(resultSet.getInt("user_id"), userId -> EnumSet.noneOf(Role.class))
                    .add(Role.valueOf(resultSet.getString("role")));
        });
        users.forEach(u -> u.setRoles(mapUserIdRoles.get(u.getId())));
        return users;
    }
